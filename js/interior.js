...
/* =================================================================
   ...
   loob ng tavern at abandonHouse").
   ...
   - `INTERIOR_ROOMS` — one entry per distinct room layout. Only one
     ...
     currently drawn there), used by BOTH tavern and abandonHouse (there's only
   ...
   - Entering (checkInteriorEntry(), called from updatePlayer() below
     only while `scene === "outside"`): either
       (a) a multiTileFootprint item with an `interior` config (tavern/
           abandonHouse) has exactly one of its otherwise-solid footprint
...
const INTERIOR_ROOM_BLUEPRINTS = {
  ...
  tavern_room: {
    ...
    // Which tiles are solid wall/frame and which are walkable floor,
    // measured off the PNG a tile at a time. '#' is wall, '.' is floor.
    // Per request ("yung mga layer 3 dapat di makalagpas jan sa
    // collissions mag red dapat") this is what stops a bed or a table
    // being dropped into the wall band: layer-3 items are refused on a
    // '#', and the placement grid shows those tiles red. Layers 4-6 —
    // the things that BELONG on a wall — are allowed there.
    //
    // Column 19 is left open from row 11 to 17 on purpose: that's the
    // doorway the indoorWarp below leads through, and walling it off
  ...
  // The Abandoned House's interior (assets/interior/asesprite/
  // abandon_room.png, 416x304) — one plain room, no inner passage. Every
  // number measured off the exported PNG rather than guessed:
  //   - the wooden frame runs x 17-23 on the left and x 392-397 on the
  //     right; the floor itself is x 24-391.
  //   - the top is black out to y 33, then the cream upper wall and its
  //     trim band down to y 88; floor starts at y 89.
  //   - the floor ends at y 293, with the bottom frame below it.
  //   - the doormat is the dark green rug at x 177-206, y 290-303.
  abandon_room: {
    image: assets.abandonRoom,
    width: 416,
    height: 304,
    // Centred on the doorway (the mat's own centre is x 191.5) and a
    // clear step above the exit zone below, so walking in doesn't
    // immediately re-trigger walking straight back out.
    spawnX: 191,
    spawnY: 248,
    // Walking DOWN onto this rectangle exits back outside. minY is
    // pulled well above the mat's true top (290) for the same reason
    // every other room's is: movement clamps the player's CENTRE to
    // `height - DRAW_SIZE/2` = 280, so anything at or past that would be
    // geometrically unreachable. 266 leaves a walkable band the player
    // can actually enter, while their feet — drawn below centre — reach
    // visibly onto the mat itself. minX/maxX are the doorway's own
    // opening with a few px of slack so the exit isn't pixel-fussy.
    exitZone: { minX: 174, maxX: 209, minY: 266, maxY: 304 },
    // The room's permanent architecture, same treatment as house_room's.
    // Outer edges run well past the room on purpose: these are inclusive
    // bounds tested against a feet POINT, so a point a hair outside the
    // room would otherwise miss every rect and read as clear floor. The
    // inner edges meet the floor exactly (89 and 293) with no sliver of
    // gap between "wall" and "floor" for the feet to sit in.
    walls: [
      { minX: -999, minY: -999, maxX: 999, maxY: 89 },  // back wall, its trim and the shelf band
      { minX: -999, minY: -999, maxX: 23, maxY: 999 },  // left frame
      { minX: 392, minY: -999, maxX: 999, maxY: 999 },  // right frame
      { minX: -999, minY: 293, maxX: 176, maxY: 999 },  // bottom wall, left of the doorway
      { minX: 207, minY: 293, maxX: 999, maxY: 999 },   // bottom wall, right of the doorway
    ],
    collisions: new Map(),
    decor: new Map(),
    // One room, nothing to warp to — all four lists stay empty, so the
    // warp simply never triggers (see checkIndoorWarpDoor()).
    indoorWarp: {
      forwardPortal: [],
      forwardSpawn: [],
...
/* ---------------- one interior per building ----------------
   Per request ("every room separate ng interior kung may gamit yung isa
   kapag sa bago walang gamit clear basta kada gawa ng house na may
   interior bagong room na interior"): the entries above are now
   BLUEPRINTS — the fixed layout of a kind of room (its art, size, spawn
   point, exit mat, walls, warp doors) — not the rooms themselves.

   Every building actually placed in the world gets its OWN room built
   from its blueprint, keyed by that building's own tile, so two houses
   of the same kind no longer share one interior: furnish one and the
   other is still empty. `INTERIOR_ROOMS` is that live registry.

   An instance copies the blueprint's fixed parts by reference (the art,
   the numbers, the wall rectangles — none of which is ever mutated) and
   gets its own FRESH `collisions` and `decor` Maps, which are the only
   per-room state there is. Those two are exactly what js/save.js already
   persists per room id, so instances save and restore with no change to
   its format — the ids are just more specific now.

   Id format is `blueprint@col,row`, which is also what lets a room be
...
// The live room for an instance id, created from its blueprint the first
// time it's asked for. Returns null only if the id names a blueprint
// that doesn't exist (a save from a version where it did, say), so
// callers can fall back rather than crash.
function getOrCreateInteriorRoom(roomId) {
  const existing = INTERIOR_ROOMS[roomId];
  if (existing) return existing;
  const blueprintId = roomId.split("@")[0];
  const blueprint = INTERIOR_ROOM_BLUEPRINTS[blueprintId];
  if (!blueprint) return null;
  const room = Object.assign({}, blueprint, {
    blueprintId,
...
// first hit wins.
//
// wallColliderPx houses (tavern/abandonHouse): the door tile is solid now,
...
// House1's door is now solid too, the same way, just via the plain
// whole-tile footprint instead of a pixel wall rect — same contact idea,
...
function checkInteriorEntry(vy) {
  ...
  for (const [key, type] of objectLayer) {
    ...
    // house: the door tile is solid now too (getObjectFootprintBlockedTiles(),
    // inventory.js), so the feet can never land exactly on it anymore —
...
// `placedCol`/`placedRow` are the tile of the specific building (or
// standalone door) that was walked into — that's what makes this ONE
// building's interior rather than a layout shared by every copy of it.
// See the block comment on INTERIOR_ROOMS above.
function enterInterior(type, placedCol, placedRow) {
  const def = itemDefs[type];
  ...
  // default tavern/abandonHouse entry point (room.spawnX/spawnY) — per
...
/* ---------------- what may be placed on a wall tile ----------------
   Per request ("yung mga layer 3 dapat di makalagpas jan sa collissions
   mag red dapat tapos ang pwede lang is yung upper object at walls at
   ceiling yung 4,5 at 6 layers").

   A room's solid parts — its wall band, its frame — are floor you can't
   stand on, so nothing that stands on the FLOOR belongs there either: no
   beds, tables, chairs, barrels. Layers 4, 5 and 6 are exactly the
   things that do belong up there (something resting on a shelf, a window
   or poster ON the wall, a ceiling tile), so they're allowed through.

   Two sources of "solid" are checked, because rooms describe themselves
   differently: `tileMap` (a per-tile '#'/'.' grid — the tavern, whose
   two halves and doorway don't reduce to a few rectangles) and `walls`
   (world-space rectangles — house_room, abandon_room). A room may have
   either, both, or neither. */
function isInteriorWallTile(room, col, row) {
  if (room.tileMap) {
    const line = room.tileMap[row];
    if (line === undefined) return true;        // past the top/bottom edge
    const ch = line[col];
    if (ch === undefined) return true;          // past the left/right edge
    if (ch === "#") return true;
  }
  if (room.walls) {
    // Sampled at the tile's centre, the same point the feet-based wall
    // test uses (isInteriorWallAt()).
    const x = (col + 0.5) * TILE;
    const feetY = (row + 0.5) * TILE;
    for (const w of room.walls) {
      if (x >= w.minX && x <= w.maxX && feetY >= w.minY && feetY <= w.maxY) return true;
    }
  }
...
// Layers 4 (things on top of furniture), 5 (wall decor) and 6 (ceiling)
// are the ones allowed onto a wall tile; layer 3 and below are not.
function mayPlaceOnInteriorWall(type) {
...
function tryGrabOrPlaceIndoorItemInFront() {
  ...
  if (!foundKey) {
    // Nothing sitting exactly on either tile — but a multi-tile piece of
    // furniture (a bench, a table) only has a map entry on its ANCHOR
    // tile, so facing any of its other tiles found nothing at all. Same
    // fix as outdoors: ask which item actually COVERS the tile in front.
...
/* ---------------- indoor warp door ----------------
   ...
   the sharedHouse blueprint's indoorWarp above) — nothing triggers until
